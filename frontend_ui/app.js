...
async function askQuestion() {
    ...
    try {
        ...
        const data = await res.json();
        answerDiv.innerText = data.answer || JSON.stringify(data);
    } catch (error) {
        answerDiv.innerText = "An error occurred while getting the response.";
    } finally {
...
document.getElementById('mode-select').addEventListener('change', function () {
    const tagInput = document.getElementById('access-tags');
...
async function uploadDoc() {
    ...
    if (!mode) {
        alert("Please select a tagging mode.");
        return;
    }
    ...
    if (mode === "manual") {
        ...
        if (!tags.length) {
            alert("Please enter at least one access tag for manual mode.");
            return;
        }
    ...
    try {
        ...
        document.getElementById('upload-result').innerText = "Upload failed due to network/server error.";
    }
}
